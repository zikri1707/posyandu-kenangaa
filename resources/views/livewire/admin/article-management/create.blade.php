...
<div class="max-w-[860px] mx-auto px-4 sm:px-8 pt-2 sm:pt-8 pb-40">
    ...
    <div class="mb-8 flex items-center justify-between">
        ...
        class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-outline-variant text-sm font-bold text-on-surface-variant hover:bg-surface-container hover:text-on-surface shadow-sm transition-all">
            <span class="material-symbols-outlined text-[18px]">arrow_back</span>
            Kembali
        </a>
        <span class="text-xs font-bold text-outline-variant uppercase tracking-widest">Buat Artikel Baru</span>
    </div>
    ...
    return {
        ...
        checkSelection() {
            setTimeout(() => {
                ...
                if (rect.width === 0 && rect.height === 0) { this.showFormatBar = false; return; }
                const barWidth = 300, barHeight = 40, margin = 8;
        ...
        toggleBlockMenu(index, event) {
            ...
            this.$nextTick(() => {
                ...
                if (btn) {
                    ...
                    const menuW = 224, menuH = 380;
                    const spaceBelow = window.innerHeight - r.bottom - 8;
                    const spaceAbove = r.top - 8;
                    let top = (spaceBelow >= menuH || spaceBelow >= spaceAbove) ? r.bottom + 6 : r.top - menuH - 6;
                    top = Math.min(top, window.innerHeight - Math.min(menuH, window.innerHeight * 0.85) - 8);
                    top = Math.max(top, 8);
                    let left = r.left - 4;
                    if (left + menuW > window.innerWidth - 8) left = window.innerWidth - menuW - 8;
                    if (left < 8) left = 8;
                    this.menuStyle = `position:fixed;top:${top}px;left:${left}px;max-height:${window.innerHeight - top - 8}px;`;
                }
            });
        },
        ...
        insertImageBlockFromMenu(event, afterIndex) {
            ...
            const reader = new FileReader();
            reader.onload = (e) => {
                this.blocks.splice(afterIndex + 1, 0, { id: this.nextId++, type: 'image', src: e.target.result, caption: '' });
        ...
        insertVideoUploadBlock(event, afterIndex) {
            ...
            const newBlock = { id: this.nextId++, type: 'video', url: '', embedSrc: '', localSrc: '' };
            this.blocks.splice(afterIndex + 1, 0, newBlock);
            const nb = { id: this.nextId++, type: 'paragraph', content: '' };
            this.blocks.splice(afterIndex + 2, 0, nb);
            this.isDirty = true;
            this.$refs.videoUploadInput.value = '';
            // Set localSrc setelah splice agar Alpine deteksi perubahan
            this.$nextTick(() => {
                const vidIdx = this.blocks.findIndex(b => b.id === newBlock.id);
                if (vidIdx !== -1) {
                    this.blocks[vidIdx].localSrc = url;
        ...
        insertVideoBlock(afterIndex) {
            this.blocks.splice(afterIndex + 1, 0, { id: this.nextId++, type: 'video', url: '', embedSrc: '', localSrc: '' });
            const nb = { id: this.nextId++, type: 'paragraph', content: '' };
            this.blocks.splice(afterIndex + 2, 0, nb);
            this.isDirty = true;
        },
        ...
        removeBlock(index) {
            this.blocks.splice(index, 1);
            if (!this.blocks.length) this.blocks.push({ id: this.nextId++, type: 'paragraph', content: '' });
            this.isDirty = true;
        },
        ...
        async submitToLivewire() {
            ...
            if (!this.coverPreview)       { valid = false; }
