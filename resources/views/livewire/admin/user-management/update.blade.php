    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-10 flex items-center justify-between">
            <div>
                ...
                <p class="text-slate-500 font-medium">Ubah informasi akun untuk <span
                        class="text-blue-600 font-bold">{{ $user->name }}</span>.</p>
            </div>
            <a href="{{ route('admin.users.index') }}"
                class="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-blue-600 hover:border-blue-100 hover:bg-blue-50 transition-all shadow-sm">
        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data"
            class="space-y-8" x-data="{ role: '{{ old('role', $user->display_role_name) }}' }">
            @csrf
            @method('PUT')

            <!-- Main Form Card -->
            <div
                class="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl shadow-slate-200/60 border border-slate-50 relative overflow-hidden">
                ...
                <div class="relative grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                    ...
                    <x-forms.form-group label="Nama Lengkap" for="name" required>
                        <x-forms.text-input name="name" placeholder="Masukkan nama lengkap"
                            value="{{ old('name', $user->name) }}" required />
                    </x-forms.form-group>

                    <!-- Username -->
                    <x-forms.form-group label="Username" for="username" required>
                        <x-forms.text-input name="username" placeholder="Contoh: bidansari"
                            value="{{ old('username', $user->username) }}" required />
                    </x-forms.form-group>

                    <!-- Email -->
                    <x-forms.form-group label="Alamat Email" for="email" required>
                        <x-forms.text-input type="email" name="email" placeholder="email@posyandu.com"
                            value="{{ old('email', $user->email) }}" required />
                    </x-forms.form-group>

                    <!-- Role -->
                    <x-forms.form-group label="Peran / Role" for="role" required>
                        <x-forms.select-input name="role" placeholder="Pilih Peran" required
                            ...
                            @foreach ($availableRoles as $val => $label)
                                <option value="{{ $val }}"
                                    {{ old('role', $currentDisplayRole) == $val ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                    ...
                    <div x-show="role.includes('admin') || role.includes('kader')"
                        ...
                        <h3 class="text-lg font-black text-indigo-800 dark:text-indigo-400 flex items-center gap-2 mb-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <x-forms.form-group label="NIK (Nomor Induk Kependudukan)" for="nik">
                                <x-forms.text-input name="nik" placeholder="Masukkan 16 digit NIK"
                                    value="{{ old('nik', $user->nik) }}" maxlength="16" />
                            </x-forms.form-group>

                            <!-- TTL -->
                            <x-forms.form-group label="Tempat, Tanggal Lahir" for="ttl">
                                <x-forms.text-input name="ttl" placeholder="Contoh: Bekasi, 12 April 1990"
                                    value="{{ old('ttl', $user->ttl) }}" />
                            </x-forms.form-group>

                            <!-- Jabatan / Peran Spesifik Kader -->
                            <x-forms.form-group label="Jabatan di Posyandu" for="cadre_role">
                                <x-forms.text-input name="cadre_role" placeholder="Contoh: Ketua Kader, Bendahara, Anggota"
                                    value="{{ old('cadre_role', $user->cadre_role) }}" />
                            </x-forms.form-group>

                            <!-- Pendidikan (Selectable Cards) -->
                            <div class="md:col-span-2">
                                <label
                                    class="block text-xs font-black text-slate-400 dark:text-gray-300 uppercase tracking-widest mb-3">Pendidikan
                                    Terakhir</label>
                                    ...
                                    @foreach (['SD', 'SMP', 'SLTA', 'Diploma', 'Sarjana', 'Magister', 'Doktor'] as $edu)
                                        <button type="button"
                                            ...
                                            :class="selected === '{{ $edu }}' ?
                                                ...
                                                'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-350 border-slate-200 dark:border-slate-700 hover:border-slate-300'"
                            ...
                            <div class="md:col-span-2">
                                <label
                                    class="block text-xs font-black text-slate-400 dark:text-gray-300 uppercase tracking-widest mb-3">Alamat
                                    Lengkap</label>
                                <textarea name="alamat" rows="3" placeholder="Masukkan alamat lengkap..."
                                    class="w-full px-5 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800 dark:text-slate-100 font-medium placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 transition-all">{{ old('alamat', $user->alamat) }}</textarea>
                            </div>

                            <!-- Foto Profil / Image Upload with Instant Live Preview -->
                            <div
                                class="md:col-span-2 flex flex-col md:flex-row items-center gap-6 p-6 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 mt-4">
                                <div
                                    class="w-24 h-24 rounded-full overflow-hidden border-2 border-slate-200 dark:border-slate-600 bg-slate-50 flex-shrink-0 relative">
                                ...
                                <div class="flex-grow text-center md:text-left">
                                    ...
                                    <p class="text-xs text-slate-500 dark:text-gray-400 mb-3">Gunakan foto wajah yang jelas
                                        dengan format JPG/PNG (Maks. 2MB)</p>
                                    ...
                                    <button type="button" onclick="document.getElementById('image-upload').click()"
                                        class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-bold text-xs rounded-xl transition-all uppercase tracking-wider">
                                        Pilih Foto Kader
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="md:col-span-2 py-4">
                        ...
                        <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] mt-4 text-center">
                            Biarkan password kosong jika tidak ingin diubah</p>
                    </div>
                    ...
                    <!-- Active Status -->
                    <div
                        class="md:col-span-2 flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-900 rounded-3xl mt-4">
                        <div class="flex items-center space-x-4">
                            <div
                                class="w-12 h-12 bg-white dark:bg-gray-800 rounded-2xl flex items-center justify-center text-indigo-600 shadow-sm">
                                <i class="fas fa-user-shield text-lg"></i>
                            </div>
                            <div>
                                <h4 class="text-sm font-bold text-slate-800 dark:text-gray-200">Status Akun</h4>
                                <p class="text-xs text-slate-500 dark:text-gray-400">Ubah status akses pengguna ke sistem.
                                </p>
                            </div>
                        </div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-4">
                <a href="{{ route('admin.users.index') }}"
                    class="px-10 py-4 bg-white border border-slate-200 text-slate-500 text-xs font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-slate-50 transition-all">Batalkan</a>
                <button type="submit"
                    class="px-10 py-4 bg-indigo-600 text-white text-xs font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/20 active:scale-95 flex items-center">
