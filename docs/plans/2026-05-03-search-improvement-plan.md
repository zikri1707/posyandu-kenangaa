# Search Bar Improvement Implementation Plan

> **For Antigravity:** REQUIRED SUB-SKILL: Load executing-plans to implement this plan task-by-task.

**Goal:** Implement a global Livewire Instant Search Dropdown for Patients, Schedules, and Articles, and upgrade all search bar visuals to a premium design.

**Architecture:** Create a new `GlobalSearch` Livewire component to handle real-time results across multiple models, and apply a unified premium design system for all search inputs.

**Tech Stack:** Laravel, Livewire, Tailwind CSS, Alpine.js, Font Awesome.

---

### Task 1: Initialize GlobalSearch Livewire Component

**Files:**
- Create: `app/Livewire/GlobalSearch.php`
- Create: `resources/views/livewire/global-search.blade.php`

**Step 1: Create the component files**
Run: `php artisan make:livewire GlobalSearch`
Expected: `DONE  Livewire component [GlobalSearch] created successfully.`

**Step 2: Commit**
```bash
git add app/Livewire/GlobalSearch.php resources/views/livewire/global-search.blade.php
git commit -m "feat: initialize GlobalSearch Livewire component"
```

---

### Task 2: Implement Search Logic in GlobalSearch Class

**Files:**
- Modify: `app/Livewire/GlobalSearch.php`

**Step 1: Add properties and search logic**
```php
...
class GlobalSearch extends Component
{
    ...
    public function render()
    {
        ...
        if (strlen($this->search) >= 2) {
            $results['patients'] = Patient::where('name', 'like', '%' . $this->search . '%')
                ->orWhere('nik', 'like', '%' . $this->search . '%')
                ->limit(5)->get();

            $results['schedules'] = Schedule::where('title', 'like', '%' . $this->search . '%')
...
**Step 2: Commit**
```bash
git add app/Livewire/GlobalSearch.php
git commit -m "feat: implement multi-model search logic in GlobalSearch"
```

---

### Task 3: Design Premium Search UI with Dropdown

**Files:**
- Modify: `resources/views/livewire/global-search.blade.php`

**Step 1: Implement the UI with Alpine.js and Tailwind**
```blade
...
**Step 2: Commit**
```bash
git add resources/views/livewire/global-search.blade.php
git commit -m "feat: design premium dropdown UI for GlobalSearch"
```

---

### Task 4: Integrate GlobalSearch into Navbar

**Files:**
- Modify: `resources/views/components/layouts/ui/navbar.blade.php`

**Step 1: Replace old search bar with Livewire component**
Replace the desktop search bar section (lines 71-83) with `@livewire('global-search')`.

**Step 2: Commit**
```bash
git add resources/views/components/layouts/ui/navbar.blade.php
git commit -m "feat: integrate GlobalSearch component into top navbar"
```

---

### Task 5: Improve Local Search Visuals (Global CSS)

**Files:**
- Modify: `resources/css/app.css`

**Step 1: Add premium search input utilities**
```css
@layer components {
    .search-input-premium {
        @apply h-11 pl-11 pr-4 rounded-2xl border border-slate-200 bg-white 
               text-slate-700 placeholder-slate-400 text-[14px] font-bold 
               focus:outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-400 
...
git commit -m "style: add premium search input utility class"
```

---

### Task 6: Apply Premium Styles to Module Search Bars

**Files:**
- Modify: `resources/views/livewire/admin/patient-management/index.blade.php`
- Modify: `resources/views/livewire/admin/user-management/index.blade.php`
- Modify: `resources/views/livewire/admin/schedule-management/index.blade.php`
- (And others found in Task 1 analysis)

**Step 1: Apply the new utility class to search inputs in management views**

**Step 2: Commit**
```bash
git commit -m "style: apply premium search styles to management modules"
```
